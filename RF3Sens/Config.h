...
//#define Algo_TimedMaxPix      // режим обнаружения минимума регулирования мощности лазера основаный на ожидаемом времени попадания пятна в центр датчика,
                              // с последующим удержанием найденой мощности и ожиданием вхождения лазера в поле зрения по MaxPix
                              // экспериментальный, базируется на геометрических размерах датчика и скорости Z 3D принтера
//#define TimeBased_wait_center 200 // время в милисекундах за которое пятно перемещается от срабатывания по алгоритму Algo_MaxPix до центра датчика
//#define TimeBased_wait_second 3000 // время в милисекундах в течении которого ожидаем второго замера (на малой скорости)
//###########################################################################################
...
#if (defined(Algo_TimedMaxPix)&&!defined(laser_power_maxpix_target))
  #error "Time based Algoritm need to use laser_power_maxpix_target"
#endif
